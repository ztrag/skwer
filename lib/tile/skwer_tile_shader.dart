...
class SkwerTileShader extends StatefulWidget {
  ...
  @override
  State<SkwerTileShader> createState() => _SkwerTileShaderState();
}

class _SkwerTileShaderState extends State<SkwerTileShader>
...
class _SkwerTilePaint extends CustomPainter {
  ...
  final _SkwerTileShaderState state;
  ...
  void load() async {
    program = await FragmentProgram.fromAsset('shaders/tile_shader.frag');
  ...
  void _paintShader(Canvas canvas, Size size) {
    ...
    shader.setFloat(i++, brightness * tileOpacity * 0.9);
    shader.setFloat(i++, animations[_mode].value);
    shader.setFloat(i++, 1.0 - animations[_press].value);

    final waves = currentWaves;
