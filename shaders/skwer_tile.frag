...
vec4 tiles(vec2 pos, vec2 size, float brightness, float flash) {
    ...
    for (int i=0; i < u_n + 0.5; i++) {
        for (int j=0; j < u_n + 0.5; j++) {
            float rx = vr * rand2(vec2(i, j)) * step(0.5, i) * (1 - step(u_n - 0.5, i));
            float ry = vr * rand2(vec2(j, i)) * step(0.5, j) * (1 - step(u_n - 0.5, j));
            // TODO 0.196?
            v[i*6 + j] = vec2(0.01 + (0.98 / u_n) * i + rx, 0.01 + (0.98 / u_n) * j + ry);
        }
    }
    ...
    for (int i=0; i < u_n - 0.5; i++) {
        for (int j=0; j < u_n - 0.5; j++) {
            float t = tileRotTime(vec2(i, j) / (u_n-1));
            vec2 i0j0 = v[i*6 + j];
            vec2 i0j1 = v[i*6 + j+1];
            vec2 i1j0 = v[(i+1)*6 + j];
            vec2 i1j1 = v[(i+1)*6 + j+1];
            i0j0 += dr * rand2(i0j0);
            i0j1 += dr * rand2(i0j1);
            i1j0 += dr * rand2(i1j0);
            i1j1 += dr * rand2(i1j1);
            ...
            qq += q;
            color += vec3(tileColor(vec2(i, j) / (u_n-1)) * q);
        }
    }
