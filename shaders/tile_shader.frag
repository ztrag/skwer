...
float smoothside(vec2 a, vec2 b, vec2 p, vec2 w) {
    vec2 ap = p - a;
    vec2 bp = p - b;
...
float quad(vec2 a, vec2 b, vec2 c, vec2 d, vec2 p, vec2 w) {
...
vec4 tiles(vec2 pos, vec2 size, float brightness, float flash) {
    ...
    float sm = 0.15;// Smoothing factor
    vec2 smv = vec2(sm / size.x, sm / size.y);
    vec3 color = vec3(0.0, 0.0, 0.0);
    float b = 0.0;
    float qq = 0.0;
    for (int i=0; i < 5; i++) {
